...
const AddDelivery: FC<{ status: string }> = ({ status }) => {
  ...
  return (
    <>
      <button
        className={`${status !== "pending" ? "bg-slate-300 cursor-not-allowed text-slate-600" : "text-brand-500 border border-brand-500"} text-sm p-2 w-1/3 rounded-md self-end`}
        ...
        >
          <div className="w-full flex items-center justify-between border-b py-2 pb-4 mb-2">
            <span className="font-bold text-black/50">
              Enter the details below to start the delivery
            </span>
          ...
          <div className="w-full flex flex-col items-start justify-center gap-1 text-sm">
            ...
            <Input
              variant="outline"
              placeholder="Enter the carrier phone number"
              name="phoneNumber"
              className="!text-sm"
            />
          </div>
          ...
          <button className="w-1/2 p-2 text-white bg-brand-500 rounded-md text-sm">
            Place delivery
          </button>
        </ModalContent>
      </Modal>
