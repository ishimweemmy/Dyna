...
import { logoutUser, setUser } from "@/features/user/userReducer"
...
const useAuth = () => {
  ...
  const loginUser = async(data: z.infer<typeof LoginFormSchema>) => {
    withLoading(async() => {
      ...
      const { data: { token, roles, user } } = response.data as { data: { token: string; roles: TRole[], user: TUser }}
      
      if(response.status == 200) {
        const isAdmin = roles.find((role: TRole) => role.name === "ADMIN")
        if(isAdmin) {
          setAuthorizationToken(token, "ACCESS")
          setUser({email: user.email, firstName: user.firstName, lastName: user.lastName, phoneNumber: user.phoneNumber, id: user.id})
          navigate(location.state?.path || "/admin", { replace: true })
          toast.success("Logged in successfully!!")
        } else {
          toast.error("You are not allowed to access an admin page, contact the admin for support!! ")
        }
      }
    })
  }
